...
var volumeStep = 0;
var volumeTimer = 0;
var showVolumeConsoleFlag = true,
    isMute = false; //showVolumeConsoleFlag：是否显示音量控制；isMute：是否静音
...
function getStbNo() {
    ...
    try {
        if(keyAction.browser == 'COSHIP' || keyAction.browser == 'coship'){
            stbno = SysInfo.STBSerialNumber;
            if(!stbno){
                mpeg_open();
                stbno = mpeg_read('STBNo');
                mpeg_close();
            }
        }
        else{
...
function getCardNo(){
    //  if(window.navigator.appName=="Microsoft Internet Explorer") return "test cardid";
    var cardno;
    try {
        if(keyAction.browser == 'COSHIP' || keyAction.browser == 'coship'){
            cardno = CA.serialNumber
            if(!cardno){
                mpeg_open();
                cardno=mpeg_read('cardId'); 
            } 
        }
        else{
            mpeg_open();
 ...
 * @param EndTime
 */
function vodFullPlay(movieAssetId, movirTitle, playPos, StartTime, EndTime, type) {
    mpeg_open();
    mpeg_write("MpegWidowState", "N");
    if (type == "small") {
        mpeg_write('CableMpegWindowCreate', '115,105,650,365'); //创建点播节目窗口，指定位置大小
    } else {
